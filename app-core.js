...
/* ===== quote-workflow.js ===== */
(function(){
  const STATUS_OPTIONS=['Draft','Sent','Approved','Completed','Cancelled'];let quoteNumber='',quoteStatus='Draft';const byId=id=>document.getElementById(id),pad=n=>String(n).padStart(2,'0'),normalVehicle=value=>String(value||'').toUpperCase().replace(/\s+/g,'').trim();
  function generateQuoteNumber(){const d=new Date(),date=`${String(d.getFullYear()).slice(-2)}${pad(d.getMonth()+1)}${pad(d.getDate())}`,time=`${pad(d.getHours())}${pad(d.getMinutes())}${pad(d.getSeconds())}`;return`AUA${date}${time}`}
  function ensureMeta(){if(!quoteNumber)quoteNumber=generateQuoteNumber();if(!STATUS_OPTIONS.includes(quoteStatus))quoteStatus='Draft'}
  function ensureStyles(){if(byId('auaQuoteWorkflowStyles'))return;const style=document.createElement('style');style.id='auaQuoteWorkflowStyles';style.textContent=`.aua-quote-workflow-panel{border-left-color:#64748b}#auaQuoteNumber{background:#f8fafc;color:#475569;font-weight:700}.aua-vehicle-suggestion{margin-top:5px;display:none}.aua-vehicle-suggestion button{width:100%;text-align:left;border:1px solid #bfdbfe;background:#eff6ff;color:#1e40af;border-radius:8px;padding:7px 8px;font-size:10.5px;font-weight:700;cursor:pointer}.aua-preview-quote-ref{font-weight:700}`;document.head.appendChild(style)}
  function ensureUi(){let panel=byId('auaQuoteWorkflowPanel');if(!panel){const customerPanel=document.querySelector('.customer-panel');if(!customerPanel)return false;panel=document.createElement('div');panel.id='auaQuoteWorkflowPanel';panel.className='panel settings-panel aua-quote-workflow-panel';panel.innerHTML=`<div class="panel-title">QUOTATION DETAILS</div><div class="grid"><div><label>Quotation No.</label><input id="auaQuoteNumber" data-preserve-case readonly></div><div><label>Status</label><select id="auaQuoteStatus">${STATUS_OPTIONS.map(x=>`<option value="${x}">${x}</option>`).join('')}</select></div></div>`;customerPanel.insertAdjacentElement('afterend',panel)}ensureStyles();const status=byId('auaQuoteStatus');if(status&&status.dataset.auaStatusBound!=='1'){status.dataset.auaStatusBound='1';status.addEventListener('change',e=>{quoteStatus=e.target.value||'Draft';window.auaSyncWorkspaceHeader?.()})}const meta=document.querySelector('.paper .meta');if(meta&&!byId('pQuoteNumberWrap')){const row=document.createElement('div');row.id='pQuoteNumberWrap';row.className='aua-preview-quote-ref';row.innerHTML='Quotation No.: <span id="pQuoteNumber"></span>';meta.prepend(row)}injectVehicleLookup();syncUi();return true}
  function syncUi(){ensureMeta();const q=byId('auaQuoteNumber'),s=byId('auaQuoteStatus'),p=byId('pQuoteNumber');if(q&&q.value!==quoteNumber)q.value=quoteNumber;if(s&&s.value!==quoteStatus)s.value=quoteStatus;if(p&&p.textContent!==quoteNumber)p.textContent=quoteNumber;window.auaSyncWorkspaceHeader?.()}
  function records(){try{return typeof getRecent==='function'?(getRecent()||[]):[]}catch{return[]}}
  function markAutofill(input){if(input&&String(input.value||'')!=='')input.dataset.auaHistoryAutofill='1'}
  function injectVehicleLookup(){const input=byId('vehicle');if(!input||byId('auaVehicleLookupList'))return;const list=document.createElement('datalist');list.id='auaVehicleLookupList';document.body.appendChild(list);input.setAttribute('list','auaVehicleLookupList');const suggestion=document.createElement('div');suggestion.id='auaVehicleSuggestion';suggestion.className='aua-vehicle-suggestion';input.insertAdjacentElement('afterend',suggestion);const refreshList=()=>{const seen=new Set(),items=[];records().slice().sort((a,b)=>Number(b.ts||0)-Number(a.ts||0)).forEach(record=>{const vehicle=String(record?.data?.vehicle||'').trim(),key=normalVehicle(vehicle);if(!key||seen.has(key))return;seen.add(key);items.push(vehicle)});list.innerHTML=items.slice(0,100).map(v=>`<option value="${String(v).replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"></option>`).join('')};const updateSuggestion=()=>{refreshList();const key=normalVehicle(input.value);if(!key){suggestion.style.display='none';suggestion.innerHTML='';return}const matches=records().filter(r=>normalVehicle(r?.data?.vehicle)===key).sort((a,b)=>Number(b.ts||0)-Number(a.ts||0)),record=matches[0];if(!record){suggestion.style.display='none';suggestion.innerHTML='';return}const d=record.data||{},bits=[d.customer,d.model].filter(Boolean).join(' · ');suggestion.innerHTML=`<button type="button">Use previous details${bits?`: ${escapeHtml(bits)}`:''}</button>`;suggestion.style.display='block';suggestion.querySelector('button').onclick=()=>{const customer=byId('customer'),phone=byId('phone'),model=byId('model');if(d.customer&&customer){customer.value=d.customer;markAutofill(customer)}if(d.phone&&phone){phone.value=d.phone;markAutofill(phone)}if(d.model&&model){model.value=d.model;markAutofill(model)}suggestion.style.display='none';if(typeof upd==='function')upd()}};input.addEventListener('focus',refreshList);input.addEventListener('input',updateSuggestion)}
  function escapeHtml(value){return String(value??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]))}
  function installHooks(){if(typeof state!=='function'||typeof loadRecord!=='function'||typeof newQuote!=='function'||typeof duplicateQuote!=='function')return false;if(state.__auaQuoteWorkflow)return true;const baseState=state,baseLoadRecord=loadRecord,baseNewQuote=newQuote,baseDuplicateQuote=duplicateQuote;ensureMeta();state=function(){const data=baseState();ensureMeta();data.quoteNumber=quoteNumber;data.status=quoteStatus;return data};state.__auaQuoteWorkflow=true;loadRecord=function(data){quoteNumber=String(data?.quoteNumber||'').trim()||generateQuoteNumber();quoteStatus=STATUS_OPTIONS.includes(data?.status)?data.status:'Draft';const result=baseLoadRecord(data);setTimeout(syncUi,0);return result};newQuote=function(){const before=JSON.stringify(baseState()),previousNumber=quoteNumber,previousStatus=quoteStatus,result=baseNewQuote.apply(this,arguments);setTimeout(()=>{const after=JSON.stringify(baseState());if(after!==before){quoteNumber=generateQuoteNumber();quoteStatus='Draft'}else{quoteNumber=previousNumber;quoteStatus=previousStatus}syncUi()},0);return result};duplicateQuote=function(){const result=baseDuplicateQuote.apply(this,arguments);quoteNumber=generateQuoteNumber();quoteStatus='Draft';setTimeout(syncUi,0);return result};return true}
  function install(){const hooks=installHooks(),ui=ensureUi();if(!hooks||!ui){setTimeout(install,120);return}syncUi()}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install);else install()
})();
...
(function(){
  const LOCAL_KEY='auaRecentQuotesV1';let clientPromise=null,installed=false;const clone=value=>{try{return JSON.parse(JSON.stringify(value))}catch{return value}},text=value=>String(value??'').trim(),quoteKey=data=>data?.quoteNumber?`quote|${text(data.quoteNumber).toLowerCase()}`:'',legacyKey=data=>{const vehicle=text(data?.vehicle).toLowerCase(),customer=text(data?.customer).toLowerCase();return vehicle?`${customer}|${vehicle}`:[customer,data?.date].map(v=>text(v).toLowerCase()).join('|')},rawRecords=()=>{try{return typeof getRecent==='function'?(getRecent()||[]):[]}catch{return[]}};
  function cloudStatus(message,tone='normal'){const el=document.getElementById('cloudStatus');if(!el)return;el.textContent=message;el.dataset.tone=tone}
  async function cloudClient(){if(clientPromise)return clientPromise;clientPromise=(async()=>{if(!window.supabase?.createClient)throw new Error('Online storage library is unavailable.');const response=await fetch('./online-storage.js',{cache:'force-cache'});if(!response.ok)throw new Error('Online storage configuration could not be read.');const source=await response.text(),url=source.match(/\burl\s*:\s*'([^']+)'/)?.[1],key=source.match(/\bkey\s*:\s*'([^']+)'/)?.[1];if(!url||!key)throw new Error('Online storage configuration is unavailable.');return window.supabase.createClient(url,key)})();return clientPromise}
  async function sessionAndClient(){const client=await cloudClient(),{data,error}=await client.auth.getSession();if(error)throw error;const user=data?.session?.user;if(!user)throw new Error('Sign in before saving a quotation.');return{client,user}}
  function promoteLocal(data,total){const key=quoteKey(data);if(!key)return;let list=[];try{list=JSON.parse(localStorage.getItem(LOCAL_KEY)||'[]')}catch{}const legacy=legacyKey(data),quote=text(data.quoteNumber).toLowerCase();list=list.filter(record=>{const existingQuote=text(record?.data?.quoteNumber).toLowerCase();return String(record?.key||'')!==legacy&&String(record?.key||'')!==key&&(!quote||existingQuote!==quote)});list.unshift({key,ts:Date.now(),total:Number(total||0),data:clone(data)});localStorage.setItem(LOCAL_KEY,JSON.stringify(list.slice(0,100)))}
  async function cleanupLegacy(client,key,alias){if(!alias||alias===key)return false;try{const{data:source}=await client.from('quotations').select('pdf_path').eq('record_key',alias).maybeSingle();if(source?.pdf_path)await client.from('quotations').update({pdf_path:source.pdf_path}).eq('record_key',key);const{error}=await client.from('quotations').delete().eq('record_key',alias);if(error)throw error;return true}catch(error){console.warn('Legacy quotation cleanup was deferred.',error);return false}}
  async function persistUnique(data,total,sourceKey){const key=quoteKey(data);if(!key)throw new Error('Quotation number is missing. Start a fresh quotation and try again.');const{client,user}=await sessionAndClient(),alias=sourceKey||legacyKey(data);let pdfPath=null;if(alias&&alias!==key){const{data:source,error:sourceError}=await client.from('quotations').select('pdf_path').eq('record_key',alias).maybeSingle();if(sourceError&&sourceError.code!=='PGRST116')console.warn('Unable to read existing quotation file link.',sourceError);pdfPath=source?.pdf_path||null}const row={user_id:user.id,record_key:key,customer:text(data.customer),vehicle:text(data.vehicle),quote_date:data.date||null,model:text(data.model),total:Number(total||0),data:clone(data),updated_at:new Date().toISOString()};if(pdfPath)row.pdf_path=pdfPath;const{error}=await client.from('quotations').upsert(row,{onConflict:'record_key'});if(error)throw error;if(alias&&alias!==key)await cleanupLegacy(client,key,alias);return true}
  async function explicitSave(){try{window.AUAQuotationAudit?.stampSave?.();try{if(typeof S!=='undefined'&&typeof rememberItem==='function')S.forEach(section=>(section.items||[]).forEach(rememberItem))}catch{}const data=clone(state()),total=Number(totals()?.grand||0);cloudStatus('Saving quotation…');await persistUnique(data,total);promoteLocal(data,total);cloudStatus(`Saved quotation${data.vehicle?' for '+data.vehicle:''}.`,'success');window.AUAUnsavedProtection?.markClean?.();return true}catch(error){cloudStatus(error?.message||'Unable to save quotation.','error');return false}}
  async function setArchived(record,archived){if(!record?.data)return false;const data=clone(record.data);data.archived=!!archived;if(archived)data.archivedAt=new Date().toISOString();else delete data.archivedAt;if(quoteKey(data)){await persistUnique(data,record.total,record.key);promoteLocal(data,record.total)}else{const{client}=await sessionAndClient(),{error}=await client.from('quotations').update({data,updated_at:new Date().toISOString()}).eq('record_key',record.key);if(error)throw error;let list=[];try{list=JSON.parse(localStorage.getItem(LOCAL_KEY)||'[]')}catch{}list.forEach(item=>{if(String(item?.key||'')===String(record.key||''))item.data=clone(data)});localStorage.setItem(LOCAL_KEY,JSON.stringify(list))}return true}
  async function cleanupCloudDuplicates(){const{client}=await sessionAndClient(),{data,error}=await client.from('quotations').select('record_key,data').limit(500);if(error)throw error;const rows=data||[],preferred=new Map();rows.forEach(row=>{const ref=text(row?.data?.quoteNumber).toLowerCase();if(ref&&String(row.record_key||'').startsWith('quote|'))preferred.set(ref,row.record_key)});let removed=0;for(const row of rows){const ref=text(row?.data?.quoteNumber).toLowerCase(),target=ref?preferred.get(ref):'';if(!target||target===row.record_key)continue;if(await cleanupLegacy(client,target,row.record_key))removed++}return removed}
  function installSaveHooks(){if(installed)return true;if(typeof state!=='function'||typeof totals!=='function')return false;saveRecent=function(){return false};saveRecord=function(){return explicitSave()};if(typeof window.__auaBaseMakePdfBlob==='function')window.makePdfBlob=window.__auaBaseMakePdfBlob;const cloudSave=document.getElementById('cloudSave');if(cloudSave){cloudSave.textContent='Save Quotation';cloudSave.onclick=()=>saveRecord()}const mainSave=document.querySelector('button[onclick*="saveRecord"]');if(mainSave)mainSave.textContent='Save Quotation';installed=true;return true}
  function preferredQuoteKeys(){const map=new Map();rawRecords().forEach(record=>{const ref=text(record?.data?.quoteNumber).toLowerCase();if(!ref)return;const current=map.get(ref);if(!current||String(record?.key||'').startsWith('quote|'))map.set(ref,String(record?.key||''))});return map}
  function ensureHistoryUi(){const advanced=document.getElementById('auaHistoryAdvanced');if(advanced&&!document.getElementById('auaHistArchive')){const field=document.createElement('div');field.className='aua-history-filter-field';field.innerHTML='<label>Records</label><select id="auaHistArchive"><option value="active">Active</option><option value="archived">Archived</option><option value="all">All</option></select>';const reset=document.getElementById('auaHistReset');advanced.insertBefore(field,reset||null);document.getElementById('auaHistArchive').addEventListener('change',decorateHistory)}if(!document.getElementById('auaIntegrityStyles')){const style=document.createElement('style');style.id='auaIntegrityStyles';style.textContent='.aua-history-quote-ref{display:block;margin-top:4px;font-size:8.8px;font-weight:700;color:#64748b;letter-spacing:.02em}.aua-history-row[hidden]{display:none!important}.aua-history-archive-note{display:block;margin-top:4px;font-size:8.5px;font-weight:800;color:#b45309}';document.head.appendChild(style)}}
  function decorateHistory(){const overlay=document.getElementById('auaHistoryOverlay');if(!overlay||overlay.hidden)return;ensureHistoryUi();const mode=document.getElementById('auaHistArchive')?.value||'active',preferred=preferredQuoteKeys();overlay.querySelectorAll('.aua-history-row[data-aua-key]').forEach(row=>{const record=rawRecords().find(r=>String(r?.key||'')===String(row.dataset.auaKey||''));if(!record){row.hidden=false;return}const ref=text(record?.data?.quoteNumber),archived=!!record?.data?.archived,preferredKey=ref?preferred.get(ref.toLowerCase()):'',duplicate=!!preferredKey&&preferredKey!==String(record.key||''),modeHidden=mode==='active'?archived:mode==='archived'?!archived:false;row.hidden=duplicate||modeHidden;const holder=row.children?.[1];if(holder&&ref){let label=holder.querySelector('.aua-history-quote-ref');if(!label){label=document.createElement('span');label.className='aua-history-quote-ref';holder.appendChild(label)}label.textContent=ref}if(holder){let note=holder.querySelector('.aua-history-archive-note');if(archived){if(!note){note=document.createElement('span');note.className='aua-history-archive-note';holder.appendChild(note)}note.textContent='ARCHIVED'}else note?.remove()}});const selected=overlay.querySelector('.aua-history-row.selected[data-aua-key]'),action=document.getElementById('auaHistoryDelete');if(selected&&action){const record=rawRecords().find(r=>String(r?.key||'')===String(selected.dataset.auaKey||''));if(record){const archived=!!record?.data?.archived;action.textContent=archived?'Restore':'Archive';action.classList.remove('aua-history-delete');action.onclick=async()=>{action.disabled=true;const old=action.textContent;action.textContent=archived?'Restoring…':'Archiving…';try{await setArchived(record,!archived);document.getElementById('auaHistoryRefresh')?.click()}catch(error){alert(error?.message||'Unable to update archive status.');action.disabled=false;action.textContent=old}}}}const visible=Array.from(overlay.querySelectorAll('.aua-history-row[data-aua-key]')).filter(row=>!row.hidden&&row.style.display!=='none').length,count=document.getElementById('auaHistoryCount');if(count&&!String(document.getElementById('auaHistorySearch')?.value||'').trim())count.textContent=`${visible} shown`}
  window.AUACloudIntegrity={setArchived,saveQuotation:explicitSave,cleanupCloudDuplicates,isArchived:record=>!!record?.data?.archived,uniqueKeyForData:quoteKey};
  function start(){installSaveHooks();ensureHistoryUi();decorateHistory();document.addEventListener('aua-history-updated',decorateHistory);document.getElementById('cloudRecordsTab')?.addEventListener('click',()=>{requestAnimationFrame(decorateHistory);setTimeout(async()=>{try{const removed=await cleanupCloudDuplicates();if(removed)document.getElementById('auaHistoryRefresh')?.click()}catch{}},700)});document.getElementById('auaHistorySearch')?.addEventListener('input',()=>requestAnimationFrame(decorateHistory))}
...
(function(){
 ...
 function displayName(raw){const value=String(raw||'').toLowerCase();if(value.includes('xavkqw@gmail.com'))return'Xavier';if(value.includes('khong.shijie@gmail.com'))return'Shijie';const match=String(raw||'').match(/^Signed in as\s+(.+)$/i);return match?.[1]||'Staff'}
 function ensureStyles(){if(byId('auaCompactTopbarStyles'))return;const style=document.createElement('style');style.id='auaCompactTopbarStyles';style.textContent=`html:not(.cloud-auth-gate) #cloudPanel{margin:0 0 11px;padding:8px 10px;border:1px solid #d9e3ee;border-radius:12px;background:rgba(255,255,255,.86);box-shadow:0 2px 8px rgba(15,39,71,.035)}html:not(.cloud-auth-gate) #cloudPanelTitle{display:none!important}html:not(.cloud-auth-gate) #cloudSignedIn:not([hidden]){display:grid;grid-template-columns:minmax(0,1fr) auto auto auto;gap:6px;align-items:center}html:not(.cloud-auth-gate) #cloudSignedIn>.toolbar{display:contents}html:not(.cloud-auth-gate) #cloudSave,html:not(.cloud-auth-gate) #cloudRecordsTab{display:none!important}html:not(.cloud-auth-gate) #cloudRecordsPanel{display:none!important}html:not(.cloud-auth-gate) #cloudUser{display:flex;min-width:0;align-items:center;gap:7px;margin:0;overflow:hidden;color:#334155;font-size:0!important;font-weight:800;white-space:nowrap}html:not(.cloud-auth-gate) #cloudUser::before{overflow:hidden;content:attr(data-aua-display);font-size:10.5px;text-overflow:ellipsis}html:not(.cloud-auth-gate) #cloudUser::after{flex:0 0 auto;content:'ONLINE';padding:3px 6px;border:1px solid #bbf7d0;border-radius:999px;background:#ecfdf3;color:#15803d;font-size:7.5px;font-weight:900;letter-spacing:.06em}html:not(.cloud-auth-gate) #cloudRefresh,html:not(.cloud-auth-gate) #cloudExport,html:not(.cloud-auth-gate) #cloudSignOut{width:auto;min-height:30px;margin:0;padding:6px 9px;border-radius:8px;background:#fff;color:#475569;border:1px solid #d5dee9;box-shadow:none;font-size:9.5px;font-weight:750;white-space:nowrap}html:not(.cloud-auth-gate) #cloudRefresh:hover,html:not(.cloud-auth-gate) #cloudExport:hover,html:not(.cloud-auth-gate) #cloudSignOut:hover{background:#f8fafc;border-color:#bdcad8;transform:none}html:not(.cloud-auth-gate) #cloudSignOut{color:#64748b}html:not(.cloud-auth-gate) #cloudStatus{margin:6px 0 0;padding:6px 8px;border-radius:8px;background:#f8fafc;font-size:9px;line-height:1.25}html:not(.cloud-auth-gate) #cloudStatus.aua-cloud-quiet{display:none!important}html:not(.cloud-auth-gate) .aua-workspace-header{top:0;margin:0 -18px 14px;padding-top:13px}@media(max-width:1120px){html:not(.cloud-auth-gate) .aua-workspace-header{margin:0 -16px 14px}}@media(max-width:620px){html:not(.cloud-auth-gate) #cloudSignedIn:not([hidden]){grid-template-columns:minmax(0,1fr) auto auto}html:not(.cloud-auth-gate) #cloudSignOut{grid-column:2/4;width:100%}}@media(max-width:520px){html:not(.cloud-auth-gate) #cloudPanel{padding:7px 8px;margin-bottom:9px}html:not(.cloud-auth-gate) #cloudUser::after{display:none}html:not(.cloud-auth-gate) #cloudRefresh,html:not(.cloud-auth-gate) #cloudExport,html:not(.cloud-auth-gate) #cloudSignOut{padding:6px 7px;font-size:9px}html:not(.cloud-auth-gate) .aua-workspace-header{margin:0 -11px 11px}}@media print{#cloudPanel{display:none!important}}`;document.head.appendChild(style)}
 function polishCloudBar(){const user=byId('cloudUser'),refresh=byId('cloudRefresh'),exportButton=byId('cloudExport'),signOut=byId('cloudSignOut'),status=byId('cloudStatus');if(!user||!refresh||!exportButton||!signOut||!status)return false;user.dataset.auaDisplay=displayName(String(user.textContent||'').trim());refresh.textContent='Refresh';exportButton.textContent='Export JSON';signOut.textContent='Sign Out';const message=String(status.textContent||'').trim(),quiet=status.dataset.tone==='success'&&(/ready/i.test(message)||/saved online/i.test(message)||/loaded\s+\d+\s+online quotation/i.test(message));status.classList.toggle('aua-cloud-quiet',quiet);return true}
 function installButtonRefreshes(){const panel=byId('cloudPanel');if(!panel||panel.dataset.auaCompactTopbarEvents)return;panel.dataset.auaCompactTopbarEvents='1';panel.addEventListener('click',()=>{setTimeout(polishCloudBar,80);setTimeout(polishCloudBar,900);setTimeout(polishCloudBar,2200)})}
 function install(attempt=0){ensureStyles();const ready=polishCloudBar();installButtonRefreshes();if(!ready&&attempt<20)setTimeout(()=>install(attempt+1),200)}
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>install(),{once:true});else install()
})();
