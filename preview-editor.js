...
(function(){
  ...
  function undoRight(){const s=undo.pop();if(s)restore(s)}
  ...
  function install(){injectStyle();markEditable();const paper=document.querySelector('.paper');if(paper)new MutationObserver(markEditable).observe(paper,{childList:true,subtree:true,attributes:true,attributeFilter:['style']});document.addEventListener('keydown',e=>{if(!(e.ctrlKey||e.metaKey)||e.key.toLowerCase()!=='z')return;if(document.activeElement?.isContentEditable)return;const inPaper=document.activeElement?.closest?.('.paper')||e.target?.closest?.('.paper');if(inPaper&&undo.length){e.preventDefault();e.stopImmediatePropagation();undoRight()}},true)}
