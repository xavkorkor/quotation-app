...
(function(){
  ...
  function injectStyle(){
    ...
    const s=document.createElement('style');s.id='auaPreviewEditStyles';s.textContent=`
      ...
      .aua-preview-tip{position:fixed;z-index:9999;background:#111827;color:#fff;font-size:10px;padding:5px 7px;border-radius:6px;pointer-events:none;opacity:.92}
  ...
  function commitCell(el,get,set,opts={}){
    ...
    el.textContent=original;el.focus();
    ...
    const finish=(cancel=false)=>{
      ...
      el.removeAttribute('contenteditable');el.classList.remove('aua-editing');delete el.dataset.auaEditing;
    ...
    el.addEventListener('keydown',key);el.addEventListener('blur',blur,{once:true});
  }
  ...
  function markSections(){
    ...
    cards.forEach((card,si)=>{
      ...
      if(title)bind(title,el=>commitCell(el,()=>sec.title,v=>{sec.title=v.toUpperCase();if(typeof render==='function')render();if(typeof upd==='function')upd()},{upper:true}),'Click to edit section name');
      ...
      lines.forEach((line,ii)=>{
        ...
        if(cells[0])bind(cells[0],el=>commitCell(el,()=>item.q??'',v=>{item.q=v;if(typeof render==='function')render();if(typeof upd==='function')upd()}),'Click to edit quantity');
        if(cells[1])bind(cells[1],el=>commitCell(el,()=>item.d??'',v=>{item.d=v.toUpperCase();if(typeof render==='function')render();if(typeof upd==='function')upd()},{upper:true}),'Click to edit description');
        if(cells[2]&&!item.included)bind(cells[2],el=>commitCell(el,()=>item.p??'',v=>{item.p=v;if(typeof render==='function')render();if(typeof upd==='function')upd()},{number:true}),'Click to edit unit price');
        if(cells[3]&&!item.included)bind(cells[3],el=>commitCell(el,()=>item.p??'',v=>{item.p=v;if(typeof render==='function')render();if(typeof upd==='function')upd()},{number:true}),'Click amount to edit unit price');
      });
      const adjust=[...card.querySelectorAll('.adjust')].find(x=>/section discount/i.test(x.textContent||''));
      if(adjust){bind(adjust,()=>{const current=Number(sec.dv||0),isPct=sec.dt==='percent';const v=prompt(isPct?'Section discount %:':'Section discount S$:',current||'');if(v===null)return;const n=Number(String(v).replace(/[^0-9.]/g,''));if(Number.isFinite(n)){sec.dv=String(n);if(typeof render==='function')render();if(typeof upd==='function')upd()}},'Click to edit section discount')}
    });
  }
