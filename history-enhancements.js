(function(){
  ...
  let selectedKey='',records=[],clientPromise=null,loading=false,visibleLimit=PAGE_SIZE,vehicleCounts=new Map();

  function reserved(record){
    const key=String(record?.record_key||record?.key||''),type=String(record?.data?.recordType||'');
    return key.startsWith(TEMPLATE_PREFIX)||key.startsWith(VEHICLE_PREFIX)||type==='job-template'||type==='vehicle-master';
  }
  function recordDate(record){
    if(Number.isFinite(record?.__auaDateMs))return new Date(record.__auaDateMs);
  ...
  function itemText(record){const out=[];(record?.data?.sections||[]).forEach(s=>{out.push(s?.title||'');(s?.items||[]).forEach(x=>out.push(x?.d||x?.desc||''))});return out.join(' ')}
  ...
  function prepare(list){
    ...
    next.forEach(record=>{
      const d=record?.data||{},vehicle=norm(d.vehicle),value=d.date||record.ts,date=new Date(typeof value==='number'?value:String(value||'').length===10?value+'T00:00:00':value);
      record.__auaDateMs=Number.isNaN(date.getTime())?0:date.getTime();
      record.__auaSearch=[d.quoteNumber,d.vehicle,d.customer,d.phone,d.model,d.mileage,itemText(record)].map(norm).join(' ');
  ...
  async function fetchCloudRecords(){
    ...
    const {data,error}=await client.from('quotations').select('record_key,total,data,updated_at,pdf_path').not('record_key','like',`${TEMPLATE_PREFIX}%`).not('record_key','like',`${VEHICLE_PREFIX}%`).order('updated_at',{ascending:false}).limit(1000);
    if(error)throw error;
    const seen=new Set(),out=[];
    (data||[]).forEach(row=>{
      if(reserved(row))return;
      const key=String(row.record_key||''),quote=text(row?.data?.quoteNumber),identity=key||quote;if(!identity||seen.has(identity))return;seen.add(identity);
      out.push({key,record_key:key,ts:new Date(row.updated_at||0).getTime(),total:Number(row.total||0),data:row.data||{},pdf_path:row.pdf_path||null});
    });
    try{localStorage.setItem(RECENTKEY,JSON.stringify(out.map(({key,ts,total,data})=>({key,ts,total,data}))))}catch{}
    return prepare(out);
  }

  function addStyles(){
    if($('auaHistoryEnhancementStyles'))return;
    const style=document.createElement('style');style.id='auaHistoryEnhancementStyles';style.textContent=`
      ...
      .aua-history-controls{padding:17px 26px;background:#fff;border-bottom:1px solid #e2e8f0}.aua-history-search{min-height:46px;font-size:14px}.aua-history-filter-row{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:11px}.aua-history-chips{display:flex;gap:7px;flex-wrap:wrap}.aua-history-chip{border:1px solid #d7e0eb;background:#fff;color:#475569;border-radius:999px;padding:8px 12px;font-size:11px;font-weight:750;cursor:pointer}.aua-history-chip.active{border-color:#2563eb;background:#eff6ff;color:#1d4ed8}.aua-history-sort{width:auto;min-width:195px}
      ...
      .aua-history-more{display:flex;justify-content:center;padding:10px}.aua-history-more button{min-width:180px}
      .aua-history-preview{padding:23px;background:#fff;border-left:1px solid #e2e8f0}.aua-history-empty{min-height:260px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#94a3b8;gap:5px}.aua-history-empty b{color:#475569}.aua-history-preview h3{margin:0;color:#0f2747;font-size:22px}.aua-history-preview-sub{margin:5px 0 16px;color:#64748b;font-size:11px}.aua-history-stats{display:grid;grid-template-columns:1fr 1fr;gap:8px}.aua-history-stat{padding:9px;border:1px solid #e2e8f0;border-radius:10px;background:#f8fafc}.aua-history-stat span{display:block;color:#64748b;font-size:8.5px;font-weight:800;text-transform:uppercase}.aua-history-stat b{display:block;margin-top:2px;font-size:11px}.aua-history-items{max-height:280px;overflow:auto;border-block:1px solid #e2e8f0;margin:14px 0}.aua-history-section{padding:8px 0 4px;font-size:9px;font-weight:800;color:#475569;text-transform:uppercase}.aua-history-item{display:flex;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid #f1f5f9;font-size:10.5px}.aua-history-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px}.aua-history-actions .wide{grid-column:1/-1}.aua-history-no-results{padding:34px;text-align:center;color:#64748b;background:#fff;border:1px dashed #cbd5e1;border-radius:12px}
      #cloudRecordsPanel.aua-history-source-hidden{display:none!important}@media(max-width:900px){.aua-history-overlay{padding:0}.aua-history-shell{min-height:100vh;border-radius:0}.aua-history-main{grid-template-columns:1fr}.aua-history-preview{border-left:0;border-top:1px solid #e2e8f0}.aua-history-columns{display:none}.aua-history-row{grid-template-columns:82px minmax(0,1fr) 90px}.aua-history-model-cell,.aua-history-updated-cell{display:none}.aua-history-filter-row{align-items:stretch;flex-direction:column}.aua-history-sort{width:100%}}`;
    document.head.appendChild(style);
  }

  function makeWorkspace(){
    if($('auaHistoryOverlay'))return;
    const overlay=document.createElement('div');overlay.id='auaHistoryOverlay';overlay.className='aua-history-overlay';overlay.hidden=true;
    overlay.innerHTML=`<div class="aua-history-shell"><header class="aua-history-header"><div><div class="aua-history-kicker">ALAN'S UNITED AUTO</div><h2>Quotation History</h2><div id="auaHistoryCount" class="aua-history-count"></div></div><div class="aua-history-head-actions"><button id="auaHistoryRefresh" class="btn secondary" type="button">Refresh</button><button id="auaHistoryClose" class="btn outline" type="button">Close</button></div></header><section class="aua-history-controls"><input id="auaHistorySearch" class="aua-history-search" type="search" data-preserve-case placeholder="Search quotation no., vehicle, customer, phone, model or item…"><div class="aua-history-filter-row"><div class="aua-history-chips"><button class="aua-history-chip" data-aua-filter="today">Today</button><button class="aua-history-chip" data-aua-filter="7">7 Days</button><button class="aua-history-chip" data-aua-filter="30">30 Days</button><button class="aua-history-chip active" data-aua-filter="all">All</button></div><select id="auaHistorySort" class="aua-history-sort"><option value="updated-desc">Recently updated</option><option value="date-desc">Quotation date — newest</option><option value="date-asc">Quotation date — oldest</option><option value="total-desc">Amount — highest</option><option value="total-asc">Amount — lowest</option><option value="vehicle-asc">Vehicle number</option></select></div></section><main class="aua-history-main"><section class="aua-history-list-wrap"><div class="aua-history-columns"><span>Date</span><span>Vehicle / Customer</span><span>Model</span><span>Total</span><span>Updated</span></div><div id="auaHistoryList" class="aua-history-list"></div></section><aside id="auaHistoryPreview" class="aua-history-preview"><div class="aua-history-empty"><b>Select a quotation</b><span>Review the details here before opening it.</span></div></aside></main></div>`;
    document.body.appendChild(overlay);
    $('auaHistoryClose').onclick=closeHistory;
    $('auaHistoryRefresh').onclick=()=>syncRecords(true);
    $('auaHistorySearch').oninput=()=>{visibleLimit=PAGE_SIZE;render()};
    $('auaHistorySort').onchange=()=>{visibleLimit=PAGE_SIZE;render()};
    overlay.querySelectorAll('[data-aua-filter]').forEach(button=>button.onclick=()=>{overlay.querySelectorAll('[data-aua-filter]').forEach(x=>x.classList.remove('active'));button.classList.add('active');visibleLimit=PAGE_SIZE;render()});
    overlay.onclick=e=>{if(e.target===overlay)closeHistory()};document.addEventListener('keydown',e=>{if(e.key==='Escape'&&!overlay.hidden)closeHistory()});
  }

  function activeFilter(){return document.querySelector('[data-aua-filter].active')?.dataset.auaFilter||'all'}
  function filteredRecords(){
    const query=norm($('auaHistorySearch')?.value),filter=activeFilter(),today=new Date();today.setHours(0,0,0,0);
    const list=records.filter(record=>{const t=record.__auaDateMs||0;if(filter==='today'&&t<today.getTime())return false;if(filter==='7'&&t<today.getTime()-6*86400000)return false;if(filter==='30'&&t<today.getTime()-29*86400000)return false;return!query||String(record.__auaSearch||'').includes(query)});
  ...
  function render(){
    const list=$('auaHistoryList');if(!list)return;const all=filteredRecords(),shown=all.slice(0,visibleLimit),remaining=Math.max(0,all.length-shown.length);
    $('auaHistoryCount').textContent=remaining?`${shown.length} of ${all.length} shown · ${records.length} quotation${records.length===1?'':'s'}`:`${all.length} shown · ${records.length} quotation${records.length===1?'':'s'}`;
    ...
    list.innerHTML=all.length?rows+(remaining?`<div class="aua-history-more"><button id="auaHistoryMore" class="btn outline" type="button">Show ${Math.min(PAGE_SIZE,remaining)} more</button></div>`:''):'<div class="aua-history-no-results">No quotations match your search or filters.</div>';
    list.querySelectorAll('[data-aua-key]').forEach(row=>row.onclick=()=>selectRecord(row.dataset.auaKey));
    const more=$('auaHistoryMore');if(more)more.onclick=()=>{visibleLimit+=PAGE_SIZE;render()};
    if(selectedKey&&!all.some(r=>String(r.key)===String(selectedKey)))clearPreview();
  }
  ...
  function selectRecord(key){
    const record=records.find(r=>String(r.key)===String(key)),preview=$('auaHistoryPreview');if(!record||!preview)return;selectedKey=record.key;const d=record.data||{},same=vehicleCount(record),sections=d.sections||[];
    ...
    $('auaHistoryOpen').onclick=()=>openExactRecord(record);$('auaHistoryVehicle').onclick=()=>{$('auaHistorySearch').value=d.vehicle||'';document.querySelectorAll('[data-aua-filter]').forEach(x=>x.classList.toggle('active',x.dataset.auaFilter==='all'));visibleLimit=PAGE_SIZE;render()};$('auaHistoryDelete').onclick=()=>toggleArchive(record);
    document.querySelectorAll('#auaHistoryList .aua-history-row[data-aua-key]').forEach(row=>row.classList.toggle('selected',String(row.dataset.auaKey)===String(record.key)));
    document.dispatchEvent(new CustomEvent('aua-history-updated',{detail:{selectedKey:record.key}}));
  }
  function openExactRecord(record){
    if(!record?.data||typeof loadRecord!=='function')return;const protection=window.AUAUnsavedProtection;if(protection?.confirmDiscard&&!protection.confirmDiscard('This quotation has unsaved changes. Open the selected History quotation anyway?'))return;
    loadRecord(clone(record.data));const status=$('cloudStatus');if(status){status.textContent=`Opened${record.data?.vehicle?' '+record.data.vehicle:''}${record.data?.quoteNumber?' · '+record.data.quoteNumber:''} from History.`;status.dataset.tone='success'}closeHistory();setTimeout(()=>document.querySelector('.customer-panel')?.scrollIntoView({behavior:'smooth',block:'start'}),0);
  }
  async function toggleArchive(record){
    if(!window.AUACloudIntegrity?.setArchived){alert('Archive control is not ready yet. Please refresh and try again.');return}const button=$('auaHistoryDelete');if(button){button.disabled=true;button.textContent=record.data?.archived?'Restoring…':'Archiving…'}
    try{await window.AUACloudIntegrity.setArchived(record,!record.data?.archived);await syncRecords(false)}catch(error){alert(error?.message||'Unable to update this quotation.');if(button)button.disabled=false}
  }
  async function syncRecords(showStatus=false){
    ...
    try{
      try{
        const next=await fetchCloudRecords();records=next;if(selectedKey&&!records.some(r=>String(r.key)===String(selectedKey)))selectedKey='';render();
      }catch(error){
        console.warn('Cloud History refresh failed; using local quotation cache.',error);if(!records.length)records=localRecords();render();if(showStatus){const status=$('cloudStatus');if(status){status.textContent=error?.message||'Using cached quotation history.';status.dataset.tone='error'}}
      }
      document.dispatchEvent(new CustomEvent('aua-history-updated',{detail:{count:records.length}}));
    }finally{loading=false;if(refresh){refresh.disabled=false;refresh.textContent='Refresh'}}
  }
  function openHistory(){
    const overlay=$('auaHistoryOverlay');if(!overlay)return Promise.resolve();overlay.hidden=false;document.body.style.overflow='hidden';visibleLimit=PAGE_SIZE;
    if(!records.length)records=localRecords();clearPreview();render();setTimeout(()=>$('auaHistorySearch')?.focus(),0);
    syncRecords(false).catch(error=>console.warn('Background History refresh failed.',error));return Promise.resolve();
  }
  function closeHistory(){const overlay=$('auaHistoryOverlay');if(overlay)overlay.hidden=true;document.body.style.overflow=''}

  function install(){
    const sourceButton=$('cloudRecordsTab'),sourcePanel=$('cloudRecordsPanel');if(!sourceButton||!sourcePanel){setTimeout(install,200);return}if(sourceButton.dataset.auaHistoryEnhanced)return;sourceButton.dataset.auaHistoryEnhanced='3';sourceButton.textContent='History';sourcePanel.classList.add('aua-history-source-hidden');addStyles();makeWorkspace();sourceButton.onclick=()=>openHistory();
  }
