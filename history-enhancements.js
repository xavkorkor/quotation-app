(function(){
  ...
  const RECENTKEY='auaRecentQuotesV1',TEMPLATE_PREFIX='template|',VEHICLE_PREFIX='vehicle-master|';
  let selectedKey='',records=[],clientPromise=null,loading=false;
  ...
  function searchText(record){const d=record?.data||{};return[d.quoteNumber,d.vehicle,d.customer,d.phone,d.model,d.mileage,itemText(record)].map(norm).join(' ')}
  ...
  function localRecords(){try{return(typeof getRecent==='function'?(getRecent()||[]):[]).filter(r=>!reserved(r))}catch{return[]}}
  ...
  async function fetchCloudRecords(){
    ...
    (data||[]).forEach(row=>{
      ...
      const key=String(row.record_key||''),quote=text(row?.data?.quoteNumber),identity=key||quote;
      if(!identity||seen.has(identity))return;seen.add(identity);
  ...
  function makeWorkspace(){
    ...
    $('auaHistoryClose').onclick=closeHistory;$('auaHistoryRefresh').onclick=()=>syncRecords(true);$('auaHistorySearch').oninput=render;$('auaHistorySort').onchange=render;
    overlay.querySelectorAll('[data-aua-filter]').forEach(button=>button.onclick=()=>{overlay.querySelectorAll('[data-aua-filter]').forEach(x=>x.classList.remove('active'));button.classList.add('active');render()});
  ...
  function filteredRecords(){
    ...
    const list=records.filter(record=>{const t=recordDate(record).getTime();if(filter==='today'&&t<today.getTime())return false;if(filter==='7'&&t<today.getTime()-6*86400000)return false;if(filter==='30'&&t<today.getTime()-29*86400000)return false;return!query||searchText(record).includes(query)});
    const sort=$('auaHistorySort')?.value||'updated-desc';list.sort((a,b)=>sort==='date-desc'?recordDate(b)-recordDate(a):sort==='date-asc'?recordDate(a)-recordDate(b):sort==='total-desc'?Number(b.total||0)-Number(a.total||0):sort==='total-asc'?Number(a.total||0)-Number(b.total||0):sort==='vehicle-asc'?text(a.data?.vehicle).localeCompare(text(b.data?.vehicle),undefined,{numeric:true}):Number(b.ts||0)-Number(a.ts||0));return list;
  }
  function vehicleCount(record){const v=norm(record?.data?.vehicle);return v?records.filter(r=>norm(r?.data?.vehicle)===v).length:0}
  function render(){
    const list=$('auaHistoryList');if(!list)return;const shown=filteredRecords();$('auaHistoryCount').textContent=`${shown.length} shown · ${records.length} quotation${records.length===1?'':'s'}`;
    list.innerHTML=shown.length?shown.map(record=>{const d=record.data||{},count=vehicleCount(record);return`<button class="aua-history-row${selectedKey===record.key?' selected':''}" type="button" data-aua-key="${esc(record.key)}"><span class="aua-history-date">${esc(dateLabel(record))}</span><span><span class="aua-history-vehicle">${esc(d.vehicle||'NO VEHICLE')}</span>${count>1?`<span class="aua-history-vehicle-count">${count} records</span>`:''}<span class="aua-history-sub">${esc(d.customer||'Unnamed customer')}</span>${d.quoteNumber?`<span class="aua-history-quote">${esc(d.quoteNumber)}</span>`:''}</span><span class="aua-history-model-cell"><span class="aua-history-model">${esc(d.model||'—')}</span></span><span class="aua-history-total">S$ ${money(record.total)}</span><span class="aua-history-updated-cell"><span class="aua-history-updated">${esc(updatedLabel(record))}</span></span></button>`}).join(''):'<div class="aua-history-no-results">No quotations match your search or filters.</div>';
    list.querySelectorAll('[data-aua-key]').forEach(row=>row.onclick=()=>selectRecord(row.dataset.auaKey));if(selectedKey&&!shown.some(r=>r.key===selectedKey))clearPreview();
  }
  function clearPreview(){selectedKey='';const p=$('auaHistoryPreview');if(p)p.innerHTML='<div class="aua-history-empty"><b>Select a quotation</b><span>Review the details here before opening it.</span></div>'}
  function selectRecord(key){
    ...
    $('auaHistoryOpen').onclick=()=>openExactRecord(record);$('auaHistoryVehicle').onclick=()=>{$('auaHistorySearch').value=d.vehicle||'';document.querySelectorAll('[data-aua-filter]').forEach(x=>x.classList.toggle('active',x.dataset.auaFilter==='all'));render()};$('auaHistoryDelete').onclick=()=>toggleArchive(record);render();document.dispatchEvent(new CustomEvent('aua-history-updated',{detail:{selectedKey:record.key}}));
  }
  ...
  async function toggleArchive(record){
    ...
    try{await window.AUACloudIntegrity.setArchived(record,!record.data?.archived);selectedKey='';await syncRecords(false)}catch(error){alert(error?.message||'Unable to update this quotation.');if(button)button.disabled=false}
  }
  async function syncRecords(showStatus=false){
    ...
    try{try{records=await fetchCloudRecords()}catch(error){console.warn('Cloud History refresh failed; using local quotation cache.',error);records=localRecords();if(showStatus){const status=$('cloudStatus');if(status){status.textContent=error?.message||'Using cached quotation history.';status.dataset.tone='error'}}}selectedKey='';render();document.dispatchEvent(new CustomEvent('aua-history-updated',{detail:{count:records.length}}))}finally{loading=false;if(refresh){refresh.disabled=false;refresh.textContent='Refresh'}}
  }
  async function openHistory(){const overlay=$('auaHistoryOverlay');if(!overlay)return;overlay.hidden=false;document.body.style.overflow='hidden';clearPreview();render();await syncRecords(false);setTimeout(()=>$('auaHistorySearch')?.focus(),0)}
  function closeHistory(){const overlay=$('auaHistoryOverlay');if(overlay)overlay.hidden=true;document.body.style.overflow=''}

  function install(){
    const sourceButton=$('cloudRecordsTab'),sourcePanel=$('cloudRecordsPanel');if(!sourceButton||!sourcePanel){setTimeout(install,200);return}if(sourceButton.dataset.auaHistoryEnhanced)return;sourceButton.dataset.auaHistoryEnhanced='2';sourceButton.textContent='History';sourcePanel.classList.add('aua-history-source-hidden');addStyles();makeWorkspace();sourceButton.onclick=()=>openHistory().catch(error=>{console.error('History could not open',error);alert(error?.message||'History could not load.')});
  }
