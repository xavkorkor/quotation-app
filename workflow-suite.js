...
(function(){
  ...
  function useCustomerDetails(record){
    ...
    const box=$('auaSmartVehicleLookup');if(box)box.classList.remove('show');
  }
  ...
  function installSmartVehicle(){
    ...
    const box=document.createElement('div');box.id='auaSmartVehicleLookup';box.className='aua-smart-vehicle';
    (input.parentElement||input).appendChild(box);
    input.addEventListener('input',()=>setTimeout(renderSmartVehicle,0));
    input.addEventListener('focus',()=>setTimeout(renderSmartVehicle,0));
  ...
  function templatePanelHtml(){
    const custom=customTemplates(),all=[...BUILTIN_TEMPLATES,...custom];
  ...
  function selectedSectionIndex(){
    ...
    if(typeof S!=='undefined'&&S.length===1)return 0;
    if(typeof S==='undefined'||!S.length)return 0;
    const choices=S.map((s,i)=>`${i+1}. ${s.title||'Section'}`).join('\n'),answer=prompt(`Which section do you want to save as a template?\n\n${choices}`,'1');
    const index=Number(answer)-1;return Number.isInteger(index)&&S[index]?index:-1;
  }
  function saveCurrentSectionTemplate(){
    if(typeof S==='undefined'||!S.length)return;
    const index=selectedSectionIndex();if(index<0)return;
  ...
  function deleteTemplate(id){const list=customTemplates().filter(t=>t.id!==id);saveCustomTemplates(list);renderTemplatePanel()}
  function isBlankSection(s){return!!s&&!(s.items||[]).some(x=>text(x.d)||text(x.p))}
  function insertTemplate(id){
    if(typeof S==='undefined'||typeof section!=='function')return;
    const template=[...BUILTIN_TEMPLATES,...customTemplates()].find(t=>t.id===id);if(!template)return;
    const next=section(clone(template.section));
    if(S.length===1&&isBlankSection(S[0]))S.splice(0,1,next);else S.push(next);
    if(typeof render==='function')render();if(typeof upd==='function')upd();
    const panel=$('auaTemplatePanel');if(panel)panel.classList.remove('show');
  }
  function installTemplates(){
    ...
    const add=$('auaAddSectionTop');top.insertBefore(trigger,add||null);
  ...
  function currentBaseSnapshot(){
    if(typeof baseState!=='function')return{};
    const data=clone(baseState());delete data.revisions;delete data.revision;delete data.audit;delete data.archived;delete data.archivedAt;
    try{data._grand=Number(typeof totals==='function'?totals().grand:0)}catch{data._grand=0}
    return data;
  }
  function signatureOf(snapshot){try{return JSON.stringify(snapshot)}catch{return''}}
  function flatItems(snapshot){
    const map=new Map();(snapshot?.sections||[]).forEach((s,si)=>(s.items||[]).forEach((x,ii)=>{const d=normDesc(x.d||x.desc);if(!d)return;const key=`${si}|${d}`;map.set(key,{d,q:text(x.q??x.qty),p:Number(x.p??x.price||0),included:!!x.included,section:text(s.title),index:ii})}));return map;
  }
  function summarizeChanges(previous,current){
    if(!previous||!Object.keys(previous).length)return['Initial saved quotation'];
    const changes=[];
    const scalars=[['customer','Customer'],['vehicle','Vehicle'],['model','Model'],['phone','Phone'],['mileage','Mileage'],['date','Date'],['status','Status']];
    scalars.forEach(([key,label])=>{if(text(previous[key])!==text(current[key]))changes.push(`${label} changed`)});
    ...
    if(!changes.length)changes.push('Quotation updated');
    const unique=[...new Set(changes)];return unique.length>6?[...unique.slice(0,6),`+ ${unique.length-6} more change${unique.length-6===1?'':'s'}`]:unique;
  }
  ...
  function updateRevisionUi(){
    let strip=$('auaRevisionStrip');const panel=$('auaQuoteWorkflowPanel');
    if(panel&&!strip){strip=document.createElement('div');strip.id='auaRevisionStrip';strip.className='aua-revision-strip';panel.appendChild(strip)}
    const last=latestRevision(),dirty=revisionDirty();
    if(strip){strip.innerHTML=last?`<span>Revision</span><b>Rev ${last.rev}</b><span class="${dirty?'aua-revision-dirty':''}">${dirty?'Unsaved changes':'Saved'}</span>`:'<span>Revision</span><b>Not saved yet</b>'}
  ...
  function stageRevision(){
    const snapshot=currentBaseSnapshot(),sig=signatureOf(snapshot),last=latestRevision();
    if(last&&(last.signature||signatureOf(last.snapshot))===sig)return false;
    const rev=(Number(last?.rev)||0)+1,entry={rev,at:new Date().toISOString(),by:staffName(),summary:summarizeChanges(last?.snapshot,snapshot),snapshot,signature:sig};
    revisions.push(entry);if(revisions.length>MAX_REVISIONS)revisions=revisions.slice(-MAX_REVISIONS);updateRevisionUi();return true;
  }
  function installRevisionHooks(){
    if(revisionHooksInstalled||typeof window.state!=='function'||typeof window.loadRecord!=='function'||typeof window.saveRecord!=='function')return false;
    revisionHooksInstalled=true;
    baseState=window.state;baseLoadRecord=window.loadRecord;baseNewQuote=window.newQuote;baseDuplicateQuote=window.duplicateQuote;
    ...
    window.saveRecord=async function(){
      const before=clone(revisions);stageRevision();
      try{const result=await baseSave.apply(this,arguments);if(result===false){revisions=before;updateRevisionUi()}return result}catch(error){revisions=before;updateRevisionUi();throw error}
    };
    document.addEventListener('input',()=>requestAnimationFrame(updateRevisionUi),true);
    document.addEventListener('change',()=>requestAnimationFrame(updateRevisionUi),true);
    updateRevisionUi();return true;
  }
  ...
  function decorateHistoryRevisions(){
    ...
    const card=document.createElement('section');card.className='aua-history-revisions';
    card.innerHTML=`<div class="aua-history-revisions-title"><b>Revision History</b><span>${list.length} revision${list.length===1?'':'s'}</span></div>${list.slice().reverse().map((r,i)=>`<details class="aua-history-revision" ${i===0?'open':''}><summary>Rev ${r.rev} · ${esc(r.by||'Staff')}</summary><div class="aua-history-revision-meta">${esc(revisionDate(r.at))}</div><ul>${(r.summary?.length?r.summary:['Saved quotation']).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></details>`).join('')}`;
  ...
  function installHistoryRevisionObserver(){
    ...
    if(attach())return;new MutationObserver(()=>attach()).observe(document.body,{childList:true,subtree:true});document.addEventListener('aua-history-updated',()=>setTimeout(decorateHistoryRevisions,0));
  }
  ...
  function collectQuoteIssues(){
    ...
    try{(S||[]).forEach((sectionData,si)=>(sectionData.items||[]).forEach((itemData,ii)=>{
      const d=text(itemData.d||itemData.desc),p=text(itemData.p??itemData.price),included=!!itemData.included;
      if(!d&&!p)return;used++;
      ...
      if(d){const key=normDesc(d);if(!seen.has(key))seen.set(key,[]);seen.get(key).push(`${sectionData.title||'Section '+(si+1)} item ${ii+1}`)}
    }));}catch{}
    if(!used)errors.push('No quotation items have been entered.');
    seen.forEach((locations,description)=>{if(locations.length>1)warnings.push(`Duplicate item: ${description} appears ${locations.length} times.`)});
    ...
    const last=latestRevision();if(last&&revisionDirty())warnings.push(`Unsaved changes since Rev ${last.rev}. Save the quotation first if these changes should be recorded as a new revision.`);else if(!last)warnings.push('This quotation has not been saved yet, so no revision has been recorded.');
  ...
  async function inspectPdfBlob(blob){
    ...
    const layout=layoutInspection();const blankTrailing=actualPages>1&&layout.contentBottomMm<=296.5?actualPages-1:Math.max(0,actualPages-layout.estimatedPages-1);
  ...
  function installPdfInspection(){
    if(pdfInspectionInstalled||typeof window.makePdfBlob!=='function')return false;pdfInspectionInstalled=true;const base=window.makePdfBlob;
  ...
  function installValidationOverride(){
    window.confirmValidation=function(){const result=collectQuoteIssues();syncValidationBox(result);if(result.errors.length){showPreflight(result);return false}if(result.warnings.length)return confirm(`Ready-to-send check found ${result.warnings.length} warning${result.warnings.length===1?'':'s'}:\n\n${result.warnings.join('\n')}\n\nContinue anyway?`);return true};
  }
  async function runFullPreflight(){
    ...
    try{const blob=await window.makePdfBlob(),pdf=lastPdfInspection||await inspectPdfBlob(blob);result.pdf=pdf;if(pdf.blankTrailing>0)result.errors.push(`${pdf.blankTrailing} trailing blank PDF page detected.`);if(pdf.horizontalOverflow)result.errors.push('PDF content extends beyond the page width.')}catch(error){result.errors.push(error?.message||'Unable to generate PDF for checking.')}showPreflight(result);return result.errors.length===0;
  }
  function installReadyButton(){
    const body=$('auaActionsBody');if(!body||$('auaReadyCheckButton'))return;
    const button=document.createElement('button');button.id='auaReadyCheckButton';button.className='btn aua-ready-button';button.type='button';button.textContent='✓ Ready to Send Check';button.onclick=async()=>{button.disabled=true;const old=button.textContent;button.textContent='Checking…';try{await runFullPreflight()}finally{button.disabled=false;button.textContent=old}};body.appendChild(button);
  }
  ...
  function installLateFeatures(attempt=0){
    const revisionsReady=installRevisionHooks(),pdfReady=installPdfInspection();updateRevisionUi();
    if((!revisionsReady||!pdfReady)&&attempt<30)setTimeout(()=>installLateFeatures(attempt+1),150);
  }
